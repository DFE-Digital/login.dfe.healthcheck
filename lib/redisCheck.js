...
const getConnectionString = (key, value) => {
  ...
  if (key.toLowerCase() === CONFIG_CONNECTION_STRING_KEY && value.toString().startsWith(REDIS_PROTOCOL)) {
    return value;
  }
  ...
  if (value instanceof Object && value.type === CONFIG_REDIS_TYPE && value.params) {
    const paramValues = Object.keys(value.params)
      .filter((key) => key.toLowerCase() !== CONFIG_CONNECTION_STRING_KEY)
      .map((key) => value.params[key]);
    const connectionString = paramValues.find((value) => value.toLowerCase().startsWith(REDIS_PROTOCOL));
    if (connectionString) {
